<template>
  <div class="grid place-items-center">
    <div class="border" id="checkout-page">
...
console.log("stripe pk", stripePK);
let stripe;
let elements;
let clientSecret;
const cart = useCart();
...
onMounted(async () => {
  ...
  const initialize = async () => {
    const { data, error } = await useFetch("/api/stripe/paymentintent", {
      method: "post",
      body: {
    ...
    console.log("stripe intent", data.value);
...
async function checkStatus(secret = "") {
  let clientSecret;
  if (secret) {
    clientSecret = secret;
  } else {
